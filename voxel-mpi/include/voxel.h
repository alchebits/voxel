...
public:
	...
	// Wersje dla rownowazenia statycznego
	void voxelizationMaster_MPI_Static(int masterID, int processQuantity);
	void voxelizationSlaves_MPI_Static(int slavesID, int processQuantity);

	// Wersje dla rownowazenia dynamicznego
	void voxelizationMaster_MPI_Dynamic(int masterID, int processQuantity);
	void voxelizationSlaves_MPI_Dynamic(int slavesID, int processQuantity);

	// Wersje dla rownowazenia dynamicznego calymi wierszami obliczanie i wysylanie
	void voxelizationMaster_MPI_DynamicRows(int masterID, int processQuantity);
	void voxelizationSlaves_MPI_DynamicRows(int slavesID, int processQuantity);

	char getVoxel(unsigned x, unsigned y, unsigned z) const;
	unsigned getSizeX() const;
	unsigned getSizeY() const;
	unsigned getSizeZ() const;

private:
	void codeRowColumnLabel(unsigned row, unsigned column, unsigned* resultLabel);
	void decodeRowColumnLabel(unsigned label, unsigned* resultRow, unsigned* resultColumn);
