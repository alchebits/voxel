...
// Struktura do wczytania czystych danych z pliku, gotowych do przeslania przez interfejs MPI
class WavefrontOBJ_Data{
public:
	WavefrontOBJ_Data();
	~WavefrontOBJ_Data();

	unsigned verticesVarNumber;
	realtype* vertices;

	unsigned pointsIDsNumber;
	int* facesPointsIDs;

	unsigned linesIDsNumber;
	int* facesLinesIDs;

	unsigned trianglesIDsNumber;
	int* facesTrianglesIDs;

	unsigned rectanglesIDsNumber;
	int* facesRectanglesIDs;
};
...
public:
	WavefrontOBJ();
	WavefrontOBJ(const WavefrontOBJ_Data& wavefrontObjData);
	~WavefrontOBJ();
