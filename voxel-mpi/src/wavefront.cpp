...
WavefrontOBJ_Data::WavefrontOBJ_Data(){
	verticesVarNumber = 0;
	pointsIDsNumber = 0;
	linesIDsNumber = 0;
	trianglesIDsNumber = 0;
	rectanglesIDsNumber = 0;

	vertices = NULL;
	facesPointsIDs = NULL;
	facesLinesIDs = NULL;
	facesTrianglesIDs = NULL;
	facesRectanglesIDs = NULL;
}
WavefrontOBJ_Data::~WavefrontOBJ_Data(){
	delete vertices;
	delete facesPointsIDs;
	delete facesLinesIDs;
	delete facesTrianglesIDs;
	delete facesRectanglesIDs;
}
...
WavefrontOBJ::WavefrontOBJ(const WavefrontOBJ_Data& wavefrontObjData){
	points.push_back(new Point(0,0,0));
	firstPointAdded = false;
	maxX = maxY = maxZ = minX = minY = minZ = 0;

	for(unsigned i = 0 ; i < wavefrontObjData.verticesVarNumber ; i+=3){
		this->addPoint( &wavefrontObjData.vertices[i] ); // addPoint -> Powinno byc addVertice albo cos takieg, bo punkt to face, a tu chodzi o wierzcholek
	}

	for(unsigned i = 0 ; i < wavefrontObjData.pointsIDsNumber ; i+=1){
		this->addFace(&wavefrontObjData.facesPointsIDs[i], 1);
	}
	for(unsigned i = 0 ; i < wavefrontObjData.linesIDsNumber ; i+=2){
		this->addFace(&wavefrontObjData.facesLinesIDs[i], 2);
	}
	for(unsigned i = 0 ; i < wavefrontObjData.trianglesIDsNumber ; i+=3){
		this->addFace(&wavefrontObjData.facesTrianglesIDs[i], 3);
	}
	for(unsigned i = 0 ; i < wavefrontObjData.rectanglesIDsNumber ; i+=4){
		this->addFace(&wavefrontObjData.facesRectanglesIDs[i], 4);
	}
}
