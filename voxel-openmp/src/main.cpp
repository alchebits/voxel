...
int voxelizator(int argc, char** argv){
	...
	unsigned procNum = paramParser.getDoubleParameter("-proc", 1); // liczba uruchamianych procesow
	...
	int procCounter =  omp_get_num_procs();
	std::cout << "# Liczba rdzeni = " << procCounter << ", liczba uzywanych procesow = " << procNum << std::endl;

	startTime = omp_get_wtime();
	grid.voxelization(procNum);
	endTime = omp_get_wtime();
...
int displayHelp(){
	...
	std::cout << "# -proc <liczba> \t Liczba procesorow uzywana podczas wokselizacji (OpenMP)" << std::endl;
