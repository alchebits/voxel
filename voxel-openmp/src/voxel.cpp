...
#pragma omp parallel for private(i,j)  firstprivate(height,depth,aabb) shared(voxelsPointer, quadTreePointer) schedule(dynamic)
	for(i=0; i < width ; ++i){

		//#pragma omp parallel for private(j) firstprivate(height,depth,aabb) shared(voxelsPointer, quadTreePointer) schedule(dynamic)
		for(j=0; j < height ; ++j){
			...
			if(zList == NULL){
				...
				for(unsigned k = 0 ; k < depth ; ++k){
					realtype zc = this->calculateCellZCenter(k,aabb,depth);
					...
					if( (isectsCounter % 2) == 0 ){
						voxelsPointer[i][j][k] = false; // wewnatrz
					}else{
						voxelsPointer[i][j][k] = true; //zewnatrz
					}
				}
			}
			delete zList;
...
Point2D Grid::setTmpPoint2D(unsigned xID, unsigned yID){	// ustawia punkt na centrum kwadraciku o danych indeksach
...
realtype Grid::calculateCellZCenter(unsigned zID, BoundingBox aabb, unsigned zRess){ // zwraca centrum na rzeczywisten osi Z danego voxela w osi siatki Z
	realtype eachLength = (aabb.xyzMax.z - aabb.xyzMin.z) / zRess;
	//std::cout << "eachLength: " << gridAABB.xyzMin.z << std::endl;
	return aabb.xyzMin.z + ( zID * eachLength + (eachLength / 2));
}
